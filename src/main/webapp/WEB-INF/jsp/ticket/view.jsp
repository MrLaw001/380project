...
<h2>Book #${ticketId}: <c:out value="${ticket.subject}"/></h2>
<security:authorize access="hasRole('ADMIN') or
                principal.username=='${ticket.customerName}'">
    [<a href="<c:url value="/ticket/edit/${ticket.id}" />">Edit</a>]<br/>
</security:authorize>
<security:authorize access="hasRole('ADMIN')">
    [<a href="<c:url value="/ticket/delete/${ticket.id}" />">Delete</a>]<br/><br/>
</security:authorize>
<i>Customer Name - <c:out value="${ticket.customerName}"/></i><br/>
Book created: <fmt:formatDate value="${ticket.createTime}"
                                pattern="EEE, d MMM yyyy HH:mm:ss Z"/><br/>
Book updated: <fmt:formatDate value="${ticket.updateTime}"
